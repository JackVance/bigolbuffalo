...
def lambda_handler(event, context):
    ...
    # Polly's synthesize_speech can transform about 1,500 chars per call, so
    # split long posts into ~1,000-char blocks at sentence/word boundaries.
    textBlocks = []
    while (len(rest) > 1100):
        begin = 0
        end = rest.find(".", 1000)
    ...
    for textBlock in textBlocks:
        ...
        if "AudioStream" in response:
            with closing(response["AudioStream"]) as stream:
                ...
                # Binary append: Polly returns bytes; Python 3 requires "ab".
